...
int main(int argc, char *argv) {
  ...
  struct cache_entry mem_cache[3];
  ...
  while(1) {
    ...
    for (int i=3; i<=nfds; i++) {
      if (FD_ISSET(i, &rds)) {
        if (i == lstnr) {
          ...
          printf(" -> *    %03dB\n", data);
          char *req_line_len = strpbrk(buf, "\r\n");
          int diff = strncmp(buf, mem_cache[0].path, req_line_len-buf);
          if (diff==0) {
            wrtbl[i]=1;
            resps[i].buf = mem_cache[0].buf;
            resps[i].curr = mem_cache[0].curr;
            continue;
          }
          reqs[i].curr += data;
    ...
    for (int i=3; i<=nfds; i++) {
      if (FD_ISSET(i, &wds)) {
        ...
        char *cc = strstr(resps[i].buf, "Cache-control: max-age=");
        if (!image && gzip) {
          ...
          if (cc) {
            mem_cache[0].curr = resps[i].curr;
            mem_cache[0].buf = resps[i].buf;
            stpncpy(mem_cache[0].path, reqs[i].buf, 32);
          }
          printf(" <- *    %03dB\n\n", strlen(resps[i].buf));
        }
